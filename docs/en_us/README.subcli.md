...
For a complete reference covering every key with type, default, description, regex writing tips, and a full YAML example, see [`docs/config-file.md`](docs/config-file.md).

### Key Categories

**Profile & export policy:**

- `profile` — built-in profile name (`bypass-cn`, `global`, `direct`).
- `profile_path` — custom profile JSON file path. Overrides `profile` for export.
- `tun` — toggle TUN template mode (`true` / `false`, default `false`).

**Paths:** `output_dir`, `template_dir`, `asset_dir`.

**Core binaries** (used by `--check` and `run`):

- `core_paths.mihomo`
- `core_paths.sing_box`
- `core_paths.xray`

**Network limits:** `parallelism`, `timeout`, `retry`, `fetch_max_bytes`, `log_level`.

**Template paths:** `templates.mihomo.normal`, `templates.mihomo.tun`, `templates.sing-box.normal`, `templates.sing-box.tun`, `templates.xray.normal`, `templates.xray.tun`. Managed more easily via `subcli template ...`.

**Node management** — filter, rename, deduplicate, and sort parsed proxy nodes:

- `node_management.dedupe` — drop duplicate nodes (`true` / `false`, default `true`).
- `node_management.rename_template` — rename pattern with `{name}`, `{region}`, `{source}`, `{protocol}` placeholders (default `{name}`). Example: `[{region}] {name}`.
- `node_management.include_regex` — only keep nodes matching this ECMAScript regex.
- `node_management.exclude_regex` — drop nodes matching this ECMAScript regex.
- `node_management.sort_by` — sort order: `region,name` (default, by region priority then name), `name` (alphabetical), or `source,name` (by subscription source then name).

**Region detection & grouping** — auto-detect node regions from names and create per-region proxy groups:

- `grouping.region_rules.<REGION>` — regex pattern that matches a region code to node names. Each matching node is tagged with that region. At export time, subcli auto-creates a proxy group named after each region (plus `PROXY`, `AUTO`, and `OTHER`). See [`docs/config-file.md`](docs/config-file.md) for full explanation and regex writing tips. Built-in defaults cover `HK`, `SG`, `JP`, `TW`, `US`.

**Asset paths & URLs:** `assets.paths.<asset-key>`, `assets.urls.<asset-key>`.

All persisted relative paths are resolved relative to the config directory. Use absolute paths for `core_paths.*` to avoid ambiguity.

`export --profile <path-or-name>` overrides `profile`/`profile_path` for the current export only and does not mutate `config.yaml`.
